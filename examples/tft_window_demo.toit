...
import bitmap show *
import font show *
import font.matthew_welch.tiny as tiny
...
main:
  ...
  while true:
    ...
    if x > sq_x: x = sq_x
    if y > sq_y: y = sq_y
    if x > 10 and not alerted:
      window = DropShadowWindow 90 70 140 100 tft.landscape
        get_rgb 255 255 153
        --corner_radius = 10
        --blur_radius = 7
        --drop_distance_x = 10
        --drop_distance_y = 10
        --shadow_opacity_percent=25
      tft.add window
      window.add
        TextTexture 70 50 window.transform TEXT_TEXTURE_ALIGN_CENTER "Alert!" sans (get_rgb 0 0 0)
      alerted = true
    if x > 150 and window:
      tft.remove window
      window = null
